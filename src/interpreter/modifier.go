...
// SubtractFromVar modifies a variable by subtracting from the variable's value. For
// strings, this results in a substring from 0:length-N. For all other variable types,
// this is regular subtraction.
func SubtractFromVar(y2kVar *Y2KVar, values []string) {
	intVal := utils.SafeStrArrToInt(values)
	switch y2kVar.Type {
	case Y2KString:
...
// MultiplyVar directly modifies a variable by multiplying the value by a number. For
// strings, this results in a string that is repeated N number of times. For all other
// variable types, this is regular multiplication.
// Note that in this case, val is always treated as a number, even for string variables.
...
// DivideVar modifies a variable by dividing the value by a number (if the variable is
// numeric) or a string (if the variable is a string). For strings, this results in a
// string with all instances of the specified string removed. For all other variable
// types, this is regular division.
// String Example: "hello world!" / "o" -> "hell wrld!"
...
// ParseModify recursively builds a set of values to modify an existing variable. The
// order of values are:
// target variable ID -> function ID -> mod size -> mod value
// Once the mod size has been reached, we can pass the mod value to the desired function
// and return the timestamp back to the original caller.
func (y2k *Y2K) ParseModify(timestamp string, varMod Y2KMod) string {
	...
	if varMod.VarID == 0 {
		...
		if len(varMod.ModValue) >= int(varMod.ModSize) {
			...
			// how the modification value needs to be interpreted. By converting the mod
			// value to a slice of strings, we can pass off final interpretation of the
			// value to the actual function that is performing the modification. For example,
			// adding to a string should interpret inputs as a string ("h" + 9 == "hi"), but
			...
			varMod.ModFunction(targetVar, utils.SplitStrByN(varMod.ModValue, y2k.Digits))
